...
public class GatewayControlFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (request.getContentLengthLong() > resolveBodySizeLimit(policy)) {
    ...
    private int resolveBodySizeLimit(GatewayRoutePolicyEntity policy) {
        if (policy != null && "MASTER_CHANGE_CREATE".equalsIgnoreCase(policy.getRouteCode())) {
            return gatewayProperties.getMasterChangeBodySizeLimitBytes();
        }
        return gatewayProperties.getBodySizeLimitBytes();
    }

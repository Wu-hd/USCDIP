...
function createSteps(): CallbackStep[] {
  return [
    ...
    },
    {
      key: 'session',
      label: '建立系统会话',
      detail: '保存 token pair 并准备进入业务首页',
      status: 'waiting'
    }
  ];
}
...
export function useOidcCallback(router: Router) {
  ...
  async function processCallback(query: LocationQuery) {
    ...
    if (isOidcStateProcessed(state)) {
      ...
      title.value = '登录已完成';
      const target = resolvePostLoginRoute(getPendingOidcState()?.postLoginRedirect);
      message.value = '该回调已处理过，正在进入全国地下管网三维健康态势首页。';
      canRetry.value = false;
      isProcessing.value = false;
      clearPendingOidcState();
    ...
    try {
      ...
      const postLoginRoute = resolvePostLoginRoute(pendingState?.postLoginRedirect);
      ...
      title.value = '登录成功';
      message.value = postLoginRoute === DEFAULT_POST_LOGIN_ROUTE
        ? '本地会话已建立，正在进入全国地下管网三维健康态势首页。'
        : '本地会话已建立，正在恢复原业务页面。';
      canRetry.value = false;
      isProcessing.value = false;

      scheduleRedirect(postLoginRoute);
    } catch (error) {

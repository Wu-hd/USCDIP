...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async startLogin(postLoginTarget?: unknown) {
      this.bindSessionEvents();
      ...
      if (!descriptor?.enabled || !descriptor.authorizationUrl) {
        this.status = 'oidc-disabled';
        this.traceId = response.traceId;
        this.message = '统一身份认证服务尚未配置，请使用管理员账号登录';
        return;
      }

      savePendingOidcState(descriptor, redirectUri, postLoginTarget);

...
import { getMasterDataDetail, getMasterDataPage, getStationPipelineLayout } from '@/services/masterData';
...
const layerConfigs: Array<MapLayerConfig & { icon: typeof CircuitBoard }> = [
  ...
  },
  {
    ...
    label: '监测站点',
    shortLabel: 'STATION',
    description: '区域级管网监测站点',
    color: '#FBBF24',
    icon: MapPin,
    legendItems: [
      { label: '监测站点', color: '#FBBF24', shape: 'diamond', description: 'STATION POINT' }
    ]
  },
  {
    key: 'ALERT',
...
const assetSearchTypes: Array<{ key: AssetSearchType; label: string; shortLabel: string }> = [
  ...
  { key: 'STATION', label: '监测站点', shortLabel: 'STATION' }
];
...
const defaultLayerState: Record<GisLayerKey, MapLayerState> = {
  ...
  STATION: { visible: true, opacity: 90, legendCollapsed: false },
...
let stationLayoutLayer: L.GeoJSON | null = null;
...
const stationLayoutState = ref<PanelState>('idle');
const stationLayoutMessage = ref('');
const total = ref(0);
...
const activeSearchTypes = computed(() =>
  selectedSearchType.value === 'ALL'
    ? (['NODE', 'SEGMENT', 'FACILITY', 'DEVICE', 'STATION'] as GisObjectType[])
...
const isStationContext = computed(
  () => selectedObject.value?.objectType === 'STATION' || readStringQuery('objectType', '') === 'STATION'
);
const gis3dTarget = computed(() =>
  isStationContext.value
    ? { path: '/home', query: { view: 'hunan', province: '430000' } }
    : { path: '/home', query: gis3dQuery.value }
);
...
function isKnownGisObjectType(value: string): value is GisObjectType {
  return ['NODE', 'SEGMENT', 'FACILITY', 'DEVICE', 'STATION'].includes(value);
}
...
function cloneDefaultLayerState(): Record<GisLayerKey, MapLayerState> {
  return {
    ...
    STATION: { ...defaultLayerState.STATION },
...
function clearStationLayout() {
  if (stationLayoutLayer) {
    layerGroups.get('STATION')?.removeLayer(stationLayoutLayer);
    stationLayoutLayer = null;
  }
}

function renderStationLayout(layout: GeoJsonFeatureCollection) {
  clearStationLayout();
  if (!layout.features.length) return;
  stationLayoutLayer = markRaw(L.geoJSON(layout as never, {
    style: (feature) => ({
      color: feature?.geometry.type === 'Point' ? '#67E8F9' : '#FBBF24',
      weight: 4,
      opacity: 0.9,
      lineCap: 'round'
    }),
    ...
    onEachFeature: (feature, layer) => {
      const label = typeof feature.properties?.name === 'string' ? feature.properties.name : '站点管网要素';
      layer.bindTooltip(label, { direction: 'top', opacity: 0.9 });
    }
  }));
  layerGroups.get('STATION')?.addLayer(stationLayoutLayer);
}

async function loadSelectedStationLayout(stationId: string) {
  stationLayoutState.value = 'loading';
  stationLayoutMessage.value = '正在加载站点管网布局';
  clearStationLayout();
  try {
    const response = await getStationPipelineLayout(stationId);
    const layout = response.data?.layout;
    if (!layout || !layout.features.length) {
      stationLayoutState.value = 'empty';
      stationLayoutMessage.value = '暂无管网布局数据';
      return;
    }
    renderStationLayout(layout);
    stationLayoutState.value = 'ready';
    stationLayoutMessage.value = `已加载 ${layout.features.length} 个管网要素`;
  } catch (error) {
    stationLayoutState.value = 'error';
    stationLayoutMessage.value = error instanceof ApiClientError ? error.message : '站点管网布局加载失败';
  }
}
...
async function selectObject(object: GisObjectRecordResponse, loadDetail = true) {
  ...
  if (object.objectType === 'STATION') {
    flowLayerVisible.value = false;
    syncFlowLayerVisibility();
    void loadSelectedStationLayout(object.objectId);
  } else {
    clearStationLayout();
    stationLayoutState.value = 'idle';
    stationLayoutMessage.value = '';
  }
...
async function applyRouteContext() {
  ...
  try {
    ...
    if (response.data.objectType === 'STATION' && response.data.anchorPoint) {
      const padding = 0.025;
      await loadBboxObjects({
        minX: response.data.anchorPoint.x - padding,
        minY: response.data.anchorPoint.y - padding,
        maxX: response.data.anchorPoint.x + padding,
        maxY: response.data.anchorPoint.y + padding,
        authoritySrid: activeGisQuery.value.authoritySrid,
        displaySrid: activeGisQuery.value.displaySrid,
...
<template>
  <main class="portal-shell">
    <div class="app-frame app-viewport flex flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8">
      <header class="glass-panel flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex min-w-0 items-center gap-3">
          ...
          <RouterLink class="secondary-button focus-ring hidden sm:inline-flex" :to="gis3dTarget">
            <Layers3 class="h-4 w-4" />
            {{ isStationContext ? '返回湖南三维图' : '3D 增强' }}
          </RouterLink>
        </div>
      ...
      <section class="grid min-h-[560px] gap-4 xl:h-[calc(100svh-8.75rem)] xl:min-h-0 xl:grid-cols-[280px_minmax(0,1fr)_360px]">
        <aside class="glass-panel order-2 flex flex-col gap-4 p-4 xl:order-1 xl:min-h-0 xl:overflow-y-auto">
        ...
        <section class="glass-panel order-1 min-h-[560px] overflow-hidden p-3 xl:order-2">
          <div class="relative h-[64vh] min-h-[520px] overflow-hidden rounded-lg border border-white/10 bg-panel xl:h-full">
            ...
            >
              bbox {{ activeGisQuery.minX }},{{ activeGisQuery.minY }} -> {{ activeGisQuery.maxX }},{{ activeGisQuery.maxY }}
        ...
        <aside
          class="glass-panel order-3 flex max-h-none flex-col overflow-hidden xl:max-h-none xl:min-h-0"
        ...
        >
          ...
          <div v-if="selectedObject" class="min-h-0 flex-1 overflow-y-auto p-4">
            ...
            <section v-if="activeDetailTab === 'profile'" class="mt-4 space-y-4">
              ...
              <div v-if="selectedObject.objectType === 'STATION'" class="rounded-lg border border-amber-300/20 bg-amber-400/[0.055] p-4">
                <div class="flex items-center gap-2">
                  <MapPin class="h-4 w-4 text-amber-200" aria-hidden="true" />
                  <p class="text-sm font-semibold text-white">站点管网布局</p>
                </div>
                <div class="mt-3 flex items-start gap-2 text-sm" :class="stationLayoutState === 'error' ? 'text-rose-200' : 'text-slate-300'">
                  <Loader2 v-if="stationLayoutState === 'loading'" class="mt-0.5 h-4 w-4 shrink-0 animate-spin" />
                  <DatabaseZap v-else class="mt-0.5 h-4 w-4 shrink-0" />
                  <p>{{ stationLayoutMessage || '等待加载站点管网布局' }}</p>
                </div>
              </div>
      ...
      <section class="glass-panel grid gap-3 p-4 md:grid-cols-3">
        ...
        <div class="rounded-lg border border-white/10 bg-white/[0.045] p-4">
          ...
          <RouterLink class="secondary-button focus-ring mt-3 w-full" :to="gis3dTarget">
            <Layers3 class="h-4 w-4" />
            {{ isStationContext ? '返回湖南三维图' : '进入 3D 增强' }}
          </RouterLink>
        </div>
      </section>
    </div>
